document.addEventListener("DOMContentLoaded", function () {
    let healthScores = []; // Structure to hold all health scores
    ...
    function drawStackedBarGraph(healthScores, timeArray) {
        var xValues = timeArray.slice(-5); // Take the last 5 elements
        var yValues = [
            healthScores.slice(-5).map(score => score.ph),
            healthScores.slice(-5).map(score => score.mh),
            healthScores.slice(-5).map(score => score.sh),
            healthScores.slice(-5).map(score => score.eh),
        ];
    ...
    function drawLineGraph(healthScores, timeArray) {
        var xValues = timeArray.slice(-5); // Take the last 5 elements
        ...
        var datasets = [
            {
                ...
                borderColor: "#ff7200",
                fill: false,
                data: healthScores.slice(-5).map(score => score.total),
            },
        ];
    ...
    drawLineGraph(healthScores, timeArray);
});
